...
| 游戏进程联动 | 监控原神客户端进程状态 |
| **多用户隔离** | 进程检查与终止按用户 SID 隔离，只操作当前用户进程 |
| 命令行设置 | 支持命令行参数配置各项参数 |
...
**实现方式**: 通过 P/Invoke `OpenProcessToken` + `GetTokenInformation` 获取进程所有者 SID，并通过 `LookupAccountSid` 获取可读用户名
...
- 进程终止：只终止当前用户 SID 启动的进程

**用户识别**:
- 使用 `WindowsIdentity.GetCurrent().User` 获取当前用户 SID
- 使用 P/Invoke Token API 获取目标进程所有者 SID
- 使用 `LookupAccountSid` 获取目标进程所有者可读用户名
...
**处理逻辑**:
1. 若检测到已存在同用户守护进程，终止旧进程并重新创建互斥体
2. 若不存在，创建互斥体，正常启动守护
3. **只终止当前用户 SID 启动的守护进程，避免影响其他用户**

### 2.6 内存检测功能

**检测方式**: 使用 GlobalMemoryStatusEx API 获取系统内存

**内存计算逻辑**:
- 物理内存：直接使用 `ullTotalPhys`
- 页面文件（虚拟内存）实际占用：`ullTotalPageFile - ullAvailPageFile` 减去物理已用部分
- 总内存：物理内存 + 页面文件实际占用
...
2. 计算物理已用和页面文件实际占用
3. 若内存超出阈值，**按用户终止**并重启 BetterGI.exe
4. 记录警告日志和重启日志
...
**日志示例**:
```
[2026-05-15 14:30:00.123] [BGIguard_v5.0.0] [INFO] BGIguard 启动成功 (用户:Bcmdy)
[2026-05-15 14:30:00.456] [BGIguard_v5.0.0] [INFO] BetterGI路径: D:\Games\BetterGI\BetterGI.exe
[2026-05-15 14:30:05.789] [BGIguard_v5.0.0] [INFO] 检测 14:30:05 | 内存: 45% | BetterGI: 运行 | 游戏: YuanShen
[2026-05-15 14:35:00.001] [BGIguard_v5.0.0] [WARN] BetterGI.exe 丢失 (第 1 次)
[2026-05-15 14:35:10.123] [BGIguard_v5.0.0] [INFO] 连续丢失达到阈值，正在重启... (当前用户:DESKTOP\Bcmdy, SID:S-1-5-21-...)
[2026-05-15 14:40:00.123] [BGIguard_v5.0.0] [INFO] 已终止 BetterGI.exe PID:1234 (用户:DESKTOP\Bcmdy, SID:S-1-5-21-...)
```
...
7. **多用户隔离**: 通过 P/Invoke Token API 实现进程按 SID 隔离，性能提升 100-500 倍
8. **启动参数过滤**: 保留 `cmd /c start` 独立启动方式，并过滤高风险 cmd 特殊字符
9. **重复代码提取**: 提取 `PromptForBetterGiPath()` 统一路径输入逻辑

---

## 7. 注意事项

1. **管理员权限**: 日志保存在 exe 同目录，若目录无写权限，可能需要以管理员身份运行或更换目录
2. **API 权限**: 需要 PROCESS_VM_READ 权限读取目标进程内存
3. **杀毒软件**: 可能被部分杀毒软件误报，请添加信任
4. **游戏兼容性**: 仅支持检测 `YuanShen.exe` 和 `GenshinImpact.exe` 两个进程名
5. **多用户环境**: 进程检查和终止按用户 SID 隔离，确保多用户环境下互不干扰
