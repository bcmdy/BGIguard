...
**检测内容**:
- 工作集内存 (Working Set) - 物理内存占用
- 虚拟内存 (Virtual Memory) - 虚拟地址空间占用

**默认阈值**:
| 类型 | 默认阈值 | 描述 |
|------|----------|------|
| 工作集内存 | 500 MB | 物理内存超出此值则重启 |
| 虚拟内存 | 1500 MB | 虚拟内存超出此值则重启 |

**处理逻辑**:
1. 在监控循环中定期检测 BGI.exe 进程内存使用
2. 若工作集内存超出阈值 或 虚拟内存超出阈值:
   - 记录警告日志 (包含当前内存使用量)
   - 终止 BGI.exe 进程
...
**日志示例**:
```
[2026-04-11 14:40:00.123] [WARN] BGI.exe 内存超限: 工作集=512MB, 虚拟内存=1600MB
...
**零配置设计**: 本程序无需任何配置文件，所有参数使用代码中的默认值。

若需自定义配置，可通过代码中的常量进行修改:

| 常量 | 默认值 | 描述 |
|------|--------|------|
| `MonitorIntervalMs` | 5000 | 监控间隔 (毫秒) |
| `RestartDelayMs` | 1000 | 重启前等待时间 (毫秒) |
| `WorkingSetMemoryLimitMB` | 500 | 工作集内存阈值 (MB) |
| `VirtualMemoryLimitMB` | 1500 | 虚拟内存阈值 (MB) |
| `MaxLogFiles` | 7 | 最多保留日志文件数量 |
